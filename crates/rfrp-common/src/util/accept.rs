//! 长驻监听循环的瞬时错误退避策略（服务端主监听、代理监听、vhost / Dashboard /
//! 客户端状态端点，以及 UDP 收包循环共用）。
//!
//! `accept()` / `recv_from()` 出错（`EMFILE`、握手期对端重置、fd 耗尽、UDP 因 ICMP
//! 报错等）绝大多数是**瞬时**的。这时若直接 `break` 结束循环，监听会永久失效而进程
//! 仍然健在——控制连接还在、客户端以为自己在线，用户侧却只能看到 connection refused
//! （UDP 则是"不再收包但端口仍绑定"），属于最难排查的"静默故障"。本策略把退避与
//! 日志抑制集中在一处，避免各监听循环各写一套。
