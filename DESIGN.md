...
>
> **⚠️ 有状态协议（如 SSH）建议 `pool_size = 0`**：预热会在客户端启动时就建立一条到本地服务的空闲连接。对有状态/有会话超时的服务（如 sshd 的 `LoginGraceTime`、防火墙 idle timeout），这条空闲连接可能在被用户首次使用前已被本地服务关闭；此时服务端命中池会拿到一条已失效的连接，首次连接表现为 `Connection reset by peer`（第二次连接因池已补充新鲜连接而恢复）。实测确认 SSH 场景下 `pool_size = 0` 可消除该现象；RDP 等无状态长连接服务仍建议预热以降低首连延迟。
...
**监听 accept 出错的统一策略（`util::accept::AcceptRetry`）：**

- 成功一次即清零；失败退避 100ms → 200ms → … → 1s 封顶；日志只在首次失败、每连续
  失败 60 次、以及到达阈值时输出，避免持续失败时每秒一条刷屏。
- **主监听**（控制端口）：连续失败达到 `MAX_CONSECUTIVE_ACCEPT_ERRORS`(60) 判定不可恢复，
  记 error 并以非零码退出，交服务管理器重启（避免"进程还在、端口不再接受连接"的静默故障）。
- **代理监听 / vhost / Dashboard / 客户端状态端点**：生命周期绑定在**会话或进程**上，
  因此**持续重试**而不是自行退出——瞬时错误（`EMFILE`、对端握手期重置）恢复后监听自动
  继续工作；自行退出会让这些监听永久静默失效，而控制连接与进程一切正常，极难排查。
...
**转发效率（v0.1 补强）**：UDP 数据面的每一环都做过批量化，但**线格式保持不变**（每包仍是独立的
`Length(4) + Data`），因此不需要协议版本协商：

- 服务端收包：同一唤醒内批量 drain 内核缓冲（`UDP_RECV_BATCH`），减少任务唤醒与系统调用；
- 会话队列：`UDP_SESSION_QUEUE_DEPTH`(64)，吸收突发（内存上界 = 深度 × 单包大小）；
- 服务端 → 工作连接：`UDP_WRITE_BATCH` 个数据报合并成**一次 write**（TLS 下即一个 record）；
- 工作连接 → 对端：`UdpFrameBuf` 一次 read 解析多帧（`UDP_READ_BATCH`），避免每包一次唤醒；
- 双端 UDP socket 接收缓冲 best-effort 调大（`UDP_SOCKET_RECV_BUF_BYTES`），先由内核吸收突发。

实测（release，server/client 独立进程，单会话突发 2000 × 1000B）：改用前服务端应用层丢包 1322 包、
端到端回收 1.6%；改用后丢包 65 包、回收 96.8%。RDP 典型速率（≈1000 pps）下应用层丢包为 **0**。
超出处理能力的部分仍按 UDP 语义丢弃并计入 `rfrp_udp_dropped_total`。
